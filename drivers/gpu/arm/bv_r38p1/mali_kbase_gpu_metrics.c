...
/**
 * enum gpu_metrics_ctx_flags - GPU metrics context state
 * @ACTIVE_INTERVAL_IN_WP: Application has activity in this work period
 * @INSIDE_ACTIVE_LIST: Context is linked on the device active list
 */
...
static inline bool
gpu_metrics_ctx_flag(struct kbase_gpu_metrics_ctx *gpu_metrics_ctx,
...
static inline void
gpu_metrics_ctx_flag_set(struct kbase_gpu_metrics_ctx *gpu_metrics_ctx,
			 enum gpu_metrics_ctx_flags flag)
{
...
static inline void
gpu_metrics_ctx_flag_clear(struct kbase_gpu_metrics_ctx *gpu_metrics_ctx,
			   enum gpu_metrics_ctx_flags flag)
{
	gpu_metrics_ctx->flags &= ~flag;
}
...
static inline void
validate_tracepoint_data(struct kbase_gpu_metrics_ctx *gpu_metrics_ctx,
			 u64 start_time, u64 end_time, u64 total_active)
{
	...
	WARN(start_time >= end_time,
	     ...
	     start_time, end_time, gpu_metrics_ctx->aid,
	     gpu_metrics_ctx->active_cnt);
	WARN(total_active > end_time - start_time,
	     "total_active %llu > period %llu for aid %u active_cnt %u",
	     total_active, end_time - start_time, gpu_metrics_ctx->aid,
	     gpu_metrics_ctx->active_cnt);
	WARN(gpu_metrics_ctx->prev_wp_active_end_time > start_time,
	     "previous end %llu > start %llu for aid %u active_cnt %u",
	...
	u64 current_time)
{
	...
	if (gpu_metrics_ctx->active_cnt) {
		...
		gpu_metrics_ctx_flag_clear(gpu_metrics_ctx,
					   ACTIVE_INTERVAL_IN_WP);
	}

	trace_gpu_work_period(kbdev->id, gpu_metrics_ctx->aid, start_time,
			      end_time, total_active);
	validate_tracepoint_data(gpu_metrics_ctx, start_time, end_time,
				 total_active);
	gpu_metrics_ctx->prev_wp_active_end_time = end_time;
	gpu_metrics_ctx->total_active = 0;
}

void kbase_gpu_metrics_ctx_put(
	struct kbase_device *kbdev,
	struct kbase_gpu_metrics_ctx *gpu_metrics_ctx)
{
	...
	if (gpu_metrics_ctx_flag(gpu_metrics_ctx, ACTIVE_INTERVAL_IN_WP))
		emit_tracepoint_for_active_gpu_metrics_ctx(
			kbdev, gpu_metrics_ctx, ktime_get_raw_ns());
...
kbase_gpu_metrics_ctx_get(struct kbase_device *kbdev, u32 aid)
{
	...
	list_for_each_entry(gpu_metrics_ctx, &gpu_metrics->inactive_list,
			    link) {
		if (gpu_metrics_ctx->aid == aid) {
			WARN_ON(!gpu_metrics_ctx->kctx_count);
			gpu_metrics_ctx->kctx_count++;
...
void kbase_gpu_metrics_ctx_init(
	struct kbase_device *kbdev,
	struct kbase_gpu_metrics_ctx *gpu_metrics_ctx, u32 aid)
{
	...
	INIT_LIST_HEAD(&gpu_metrics_ctx->link);
	list_add_tail(&gpu_metrics_ctx->link,
		      &kbdev->gpu_metrics.inactive_list);
}
					  ...
					  u64 timestamp_ns)
{
	...
	if (!gpu_metrics_ctx_flag(gpu_metrics_ctx, ACTIVE_INTERVAL_IN_WP)) {
		...
		gpu_metrics_ctx_flag_set(gpu_metrics_ctx,
					 ACTIVE_INTERVAL_IN_WP);
	}

	if (!gpu_metrics_ctx_flag(gpu_metrics_ctx, INSIDE_ACTIVE_LIST)) {
		list_move_tail(&gpu_metrics_ctx->link,
			       &kctx->kbdev->gpu_metrics.active_list);
					...
					u64 timestamp_ns)
{
	...
	/* Timestamp conversion can round a start and end to the same ns. */
	...
	/* Handle a delayed ACT=0 observation across adjacent work periods. */
	if (gpu_metrics_ctx->last_active_start_time ==
			gpu_metrics_ctx->first_active_start_time &&
	    gpu_metrics_ctx->prev_wp_active_end_time ==
			gpu_metrics_ctx->first_active_start_time) {
				       ...
				       u64 timestamp_ns)
{
	...
	list_for_each_entry_safe(gpu_metrics_ctx, tmp,
				 &gpu_metrics->active_list, link) {
		if (!gpu_metrics_ctx_flag(gpu_metrics_ctx,
					  ACTIVE_INTERVAL_IN_WP)) {
			bool inside_active_list = gpu_metrics_ctx_flag(
				gpu_metrics_ctx, INSIDE_ACTIVE_LIST);

			WARN_ON(!inside_active_list);
			WARN_ON(gpu_metrics_ctx->active_cnt);
			list_move_tail(&gpu_metrics_ctx->link,
				       &gpu_metrics->inactive_list);
			gpu_metrics_ctx_flag_clear(gpu_metrics_ctx,
						   INSIDE_ACTIVE_LIST);
			continue;
		}
		...
		emit_tracepoint_for_active_gpu_metrics_ctx(
			kbdev, gpu_metrics_ctx, timestamp_ns);
	}
}
...
int kbase_gpu_metrics_init(struct kbase_device *kbdev)
{
	...
	dev_info(kbdev->dev, "GPU work-period tracepoint support enabled");
	return 0;
}
