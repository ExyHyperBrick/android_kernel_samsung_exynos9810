...
{
	int index = (S6E3HF3_NR_TP - v - 1);

	if (v == S6E3HF3_NR_TP - 1) {
		output[color * 2] = ((value & 0x0100) ? 1 : 0);
		output[color * 2 + 1] = value & 0x00FF;
		if (value > 511)
			pr_err("%s, error : exceed output range tp:%d c:%d value:%d!!\n",
					__func__, index, color, value);
	} else if (v == 0) {
		/* V0 */
		output[(index + 1) * MAX_COLOR + color] = 0;

		/* VT */
		if (color == RED) {
			output[(index + 2) * MAX_COLOR] &= ~(0xF << 4);
			output[(index + 2) * MAX_COLOR] |= (value & 0xF) << 4;
		} else if (color == GREEN) {
			output[(index + 2) * MAX_COLOR] &= ~0xF;
			output[(index + 2) * MAX_COLOR] |= value & 0xF;
		} else if (color == BLUE) {
			output[(index + 2) * MAX_COLOR + 1] &= ~0xF;
			output[(index + 2) * MAX_COLOR + 1] |= value & 0xF;
		}
		if (value > 15)
			pr_err("%s, error : exceed output range tp:%d c:%d value:%d!!\n",
					__func__, index, color, value);
	} else {
		output[(index + 1) * MAX_COLOR + color] = value & 0x00FF;
		if (value > 255)
			pr_err("%s, error : exceed output range tp:%d c:%d value:%d!!\n",
					__func__, index, color, value);
	}
...
int s6e3hf3_init_common_table(struct maptbl *tbl)
{
	if (tbl == NULL) {
...
int s6e3hf3_init_gamma_table(struct maptbl *tbl)
{
	return init_subdev_gamma_table(tbl, PANEL_BL_SUBDEV_TYPE_DISP);
}

int s6e3hf3_getidx_common_maptbl(struct maptbl *tbl)
{
	return 0;
}

int s6e3hf3_getidx_dimming_maptbl(struct maptbl *tbl)
{
	int row;
	struct panel_bl_device *panel_bl;
...
#ifdef CONFIG_SUPPORT_HMD
int s6e3hf3_init_hmd_gamma_table(struct maptbl *tbl)
{
	return init_subdev_gamma_table(tbl, PANEL_BL_SUBDEV_TYPE_HMD);
}

int s6e3hf3_getidx_hmd_dimming_mtptbl(struct maptbl *tbl)
{
	int row = 0;
	struct panel_bl_device *panel_bl;
	struct panel_device *panel = (struct panel_device *)tbl->pdata;
...
{
	return s6e3hf3_getidx_brt_tbl(tbl);
}
...
int s6e3hf3_getidx_mps_table(struct maptbl *tbl)
{
	struct panel_device *panel = (struct panel_device *)tbl->pdata;
	struct panel_bl_device *panel_bl;
	int row;
...
int s6e3hf3_getidx_elvss_temp_table(struct maptbl *tbl)
{
	int row, plane;
	struct panel_info *panel_data;
	struct panel_bl_device *panel_bl;
...
int s6e3hf3_getidx_vgh_table(struct maptbl *tbl)
{
	struct panel_device *panel = (struct panel_device *)tbl->pdata;
	struct panel_info *panel_data;
	int row = 0;
...
int s6e3hf3_getidx_vint_table(struct maptbl *tbl)
{
	int row, nit;
...
int s6e3hf3_getidx_acl_onoff_table(struct maptbl *tbl)
{
	struct panel_device *panel = (struct panel_device *)tbl->pdata;
...
int s6e3hf3_getidx_hbm_onoff_table(struct maptbl *tbl)
{
	struct panel_device *panel = (struct panel_device *)tbl->pdata;
	struct panel_bl_device *panel_bl;
...
int s6e3hf3_getidx_acl_opr_table(struct maptbl *tbl)
{
	struct panel_device *panel = (struct panel_device *)tbl->pdata;
...
int s6e3hf3_getidx_dsc_table(struct maptbl *tbl)
{
...
int s6e3hf3_getidx_resolution_table(struct maptbl *tbl)
{
	int row = 0;
...
int s6e3hf3_getidx_dd_sel_table(struct maptbl *tbl)
{
	return 0;
}
...
void s6e3hf3_copy_common_maptbl(struct maptbl *tbl, u8 *dst)
{
	int idx;
	if (!tbl || !dst) {
...
void s6e3hf3_copy_elvss_temp_maptbl(struct maptbl *tbl, u8 *dst)
{
	struct panel_device *panel;
	struct panel_info *panel_data;
	int idx, ret;
...
void s6e3hf3_copy_tset_maptbl(struct maptbl *tbl, u8 *dst)
{
	struct panel_device *panel;
	struct panel_info *panel_data;
	if (!tbl || !dst)
...
void s6e3hf3_copy_copr_maptbl(struct maptbl *tbl, u8 *dst)
{
...
int s6e3hf3_init_color_coordinate_table(struct maptbl *tbl)
{
	struct mdnie_info *mdnie;
	int type, color;
...
int s6e3hf3_getidx_mdnie_maptbl(struct pkt_update_info *pktui, int offset)
{
	struct panel_device *panel = pktui->pdata;
	struct mdnie_info *mdnie = &panel->mdnie;
...
{
	return s6e3hf3_getidx_mdnie_maptbl(pktui, 0);
}

int s6e3hf3_getidx_mdnie_1_maptbl(struct pkt_update_info *pktui)
{
	return s6e3hf3_getidx_mdnie_maptbl(pktui, 1);
}

int s6e3hf3_getidx_mdnie_2_maptbl(struct pkt_update_info *pktui)
{
	return s6e3hf3_getidx_mdnie_maptbl(pktui, 2);
}
