...
static int __get_nat_bitmaps(struct f2fs_sb_info *sbi)
{
	...
	nm_i->nat_bits = kvzalloc(nm_i->nat_bits_blocks << F2FS_BLKSIZE_BITS, GFP_KERNEL);
	if (!nm_i->nat_bits)
		return -ENOMEM;
...
static int init_free_nid_cache(struct f2fs_sb_info *sbi)
{
	...
	nm_i->free_nid_bitmap =
		kvzalloc(array_size(sizeof(unsigned char *),
					   nm_i->nat_blocks),
			 GFP_KERNEL);
	...
	for (i = 0; i < nm_i->nat_blocks; i++) {
		nm_i->free_nid_bitmap[i] = kvzalloc(f2fs_bitmap_size(NAT_ENTRY_PER_BLOCK),
						    GFP_KERNEL);
	...
	nm_i->free_nid_count =
		kvzalloc(array_size(sizeof(unsigned short),
					   nm_i->nat_blocks),
			 GFP_KERNEL);
