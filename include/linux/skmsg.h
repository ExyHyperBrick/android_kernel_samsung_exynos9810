...
struct sk_psock {
	...
	/* Protects ingress queues and associated work state. */
...
int sk_msg_wait_data(struct sock *sk, struct sk_psock *psock, int flags,
		     long timeo, int *err);
int sk_msg_recvmsg(struct sock *sk, struct sk_psock *psock,
...
{
	return sk_psock(sk) &&
	       (sk->sk_prot->recvmsg == tcp_bpf_recvmsg ||
		sk->sk_prot->recvmsg == tcp_bpf_recvmsg_parser);
}
...
static inline struct sk_msg *sk_psock_dequeue_msg(struct sk_psock *psock)
{
	...
	spin_lock_bh(&psock->ingress_lock);
	msg = list_first_entry_or_null(&psock->ingress_msg,
				       struct sk_msg, list);
	if (msg)
		list_del(&msg->list);
	spin_unlock_bh(&psock->ingress_lock);
...
static inline void sk_psock_data_ready(struct sock *sk,
				       struct sk_psock *psock)
{
