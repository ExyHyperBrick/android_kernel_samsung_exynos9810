			    ...
			    loff_t *ppos)
{
	...
	while (1) {
		...
		if (pos == seq->index) {
			pr_info_ratelimited(
				"buggy seq_file .next %ps did not update position\n",
				seq->op->next);
			seq->index++;
		}
...
bool bpf_iter_prog_supported(struct bpf_prog *prog)
{
	...
	list_for_each_entry(tinfo, &targets, list) {
		...
		if (!strcmp(attach_fname + prefix_len,
...
bpf_iter_get_func_proto(enum bpf_func_id func_id,
				 ...
				 struct bpf_prog *old_prog)
{
	...
	if (link->prog->type != new_prog->type ||
	    link->prog->expected_attach_type !=
	    new_prog->expected_attach_type ||
...
int bpf_iter_link_attach(const union bpf_attr *attr, struct bpf_prog *prog)
{
	...
	bpf_link_init(&link->link, BPF_LINK_TYPE_ITER,
		      &bpf_iter_link_lops, prog);
	link->tinfo = tinfo;
	...
	if (tinfo->reg_info->attach_target) {
		err = tinfo->reg_info->attach_target(prog, &linfo,
						     &link->aux);
		if (err) {
			    ...
			    const struct bpf_iter_seq_info *seq_info)
{
	...
	if (seq_info->init_seq_private) {
		err = seq_info->init_seq_private(priv_data->target_private,
						 &link->aux);
		if (err)
...
int bpf_iter_run_prog(struct bpf_prog *prog, void *ctx)
{
	...
	rcu_read_lock();
	ret = bpf_prog_run_pin_on_cpu(prog, ctx);
	rcu_read_unlock();
