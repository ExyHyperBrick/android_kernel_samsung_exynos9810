...
struct btf_show {
	...
	__printf(2, 0) void (*showfn)(struct btf_show *show, const char *fmt,
				      va_list args);
			       ...
			       void *data)
{
	...
	if (!safe) {
		...
		show->state.status = probe_kernel_read(show->obj.safe, data,
						       size_left);
 ...
 * *type_id: id of type if it is changed within the function, 0 if not
 *
...
{
	return btf_find_field(btf, t, "bpf_timer", sizeof(struct bpf_timer),
...
btf_get_prog_ctx_type(struct bpf_verifier_log *log, struct btf *btf,
		      const struct btf_type *t,
		      enum bpf_prog_type prog_type, int arg)
{
	...
	/* prog_type is valid BPF program type. No bounds check is needed. */
	ctx_type = btf_type_member(conv_struct) +
		   bpf_ctx_convert_map[prog_type] * 2;
	/* ctx_struct is the program-side context type in vmlinux. */
...
/* Compare a function's BTF with the supplied verifier register state.
 *
 * Returns -EFAULT for a verifier bug, -EINVAL for a type mismatch or
 * unavailable BTF, and zero when the BTF and register state match.
 * Only PTR_TO_CTX and SCALAR_VALUE arguments are recognized.
 */
			     ...
			     struct bpf_reg_state *reg)
{
	...
	if (!t || !btf_type_is_func(t)) {
		/* check_btf_func() already validated this relationship. */
	...
	/* Check that BTF arguments match the types seen by the verifier. */
	for (i = 0; i < nargs; i++) {
...
out:
	 ...
	 * while callers can pass mismatched types into global functions.
	 */
	prog->aux->func_info_aux[subprog].unreliable = true;
	return -EINVAL;
}

/* Convert a global function's BTF prototype into verifier registers. */
			  ...
			  struct bpf_reg_state *reg)
{
	...
	/* Global functions currently support only scalar return values. */
	t = btf_type_by_id(btf, t->type);
	while (btf_type_is_modifier(t))
		t = btf_type_by_id(btf, t->type);
	if (!btf_type_is_int(t) && !btf_type_is_enum(t)) {
	...
	/* Convert supported BTF arguments into verifier register types. */
	for (i = 0; i < nargs; i++) {
		t = btf_type_by_id(btf, args[i].type);
		while (btf_type_is_modifier(t))
			t = btf_type_by_id(btf, t->type);
...
struct btf *btf_parse_vmlinux(void)
{
	...
	/* Find struct bpf_ctx_convert for later context type checking. */
	for (i = 1; i <= btf->nr_types; i++) {
		const struct btf_type *t;
		const char *tname;
		    ...
		    struct bpf_insn_access_aux *info)
{
	...
	for (i = 0; i < prog->aux->ctx_arg_info_size; i++) {
		...
		ctx_arg_info = &prog->aux->ctx_arg_info[i];
...
int btf_distill_func_proto(struct bpf_verifier_log *log, struct btf *btf,
			   const struct btf_type *func, const char *tname,
...
/* Compare BTFs of two functions assuming only scalars and pointers to
 * context.
...
int btf_check_type_match(struct bpf_verifier_log *log,
			 const struct bpf_prog *prog, struct btf *btf2,
			 const struct btf_type *t2)
{
...
error:
	...
	for_each_member(i, t, member) {
		...
		if (btf_member_bitfield_size(t, member)) {
			...
			/* off may be accessing a following member or doing
			 * a partial access at either end of this bitfield.
			 * Continue so it is eventually rejected as an access
			 * to a missing field.
			 */
			continue;
		}
...
__printf(2, 0)
static void btf_seq_show(struct btf_show *show, const char *fmt, va_list args)
{
