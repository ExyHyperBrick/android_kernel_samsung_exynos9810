 ...
 * - Argument preparation: data->bin_args receives a binary representation
 *   suitable for bstr_printf(), with pointers from BPF sanitized.
 *
	   ...
	   u64, flags)
{
	...
	/* This 4.9 tree predates the map-aware memcg allocator. */
	...
	/* Order timer->timer before map->usercnt. With a concurrent final
	 * user-reference release, either timer teardown observes the new timer
	 * or this check observes a zero user count.
	 */
	smp_mb();
	if (!atomic64_read(&map->usercnt)) {
		/* Maps with timers must be held by user space or pinned in bpffs. */
...
BPF_CALL_3(bpf_timer_start, struct bpf_timer_kern *, timer, u64, nsecs,
	   u64, flags)
{
	struct bpf_hrtimer *t;
	int ret = 0;
...
extern const struct bpf_func_proto bpf_task_pt_regs_proto __weak;
