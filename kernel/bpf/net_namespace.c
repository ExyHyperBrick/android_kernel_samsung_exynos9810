...
struct bpf_netns_link {
	 ...
	 * when netns is going away. Instead we rely on the pernet exit
	 * callback to clear this pointer. Must be accessed with
	 * netns_bpf_mutex held.
	 */
	struct net *net;
				       ...
				       enum netns_bpf_attach_type type)
{
	...
	run_array = rcu_dereference_protected(net->bpf.run_array[type], 1);
...
int netns_bpf_prog_attach(const union bpf_attr *attr, struct bpf_prog *prog)
{
	...
	/* Attaching a program directly is not compatible with links. */
				   ...
				   enum netns_bpf_attach_type type)
{
	...
	/* Programs attached through links cannot be detached directly. */
	if (!list_empty(&net->bpf.links[type]))
		return -EINVAL;
				 ...
				 enum netns_bpf_attach_type type)
{
	...
	/* Allow attaching only one program or link for now. */
	if (!list_empty(&net->bpf.links[type])) {
		err = -E2BIG;
		goto out_unlock;
	}
	/* Links are not compatible with attaching a program directly. */
