			     ...
			     size_t actual_size)
{
	...
	if (actual_size <= expected_size)
		return 0;
	if (uaddr.is_kernel)
		return memchr_inv(uaddr.kernel + expected_size, 0,
				  actual_size - expected_size) ? -E2BIG : 0;

	if (unlikely(!access_ok(VERIFY_READ, uaddr.user, actual_size)))
		return -EFAULT;

	addr = uaddr.user + expected_size;
	end  = uaddr.user + actual_size;
					     ...
					     enum bpf_attach_type attach_type)
{
	...
	case BPF_PROG_TYPE_CGROUP_SKB:
		/* Check unprivileged programs at attach time. */
				...
				struct bpf_prog *prog)
{
	...
	err = perf_event_set_bpf_prog(
		event, prog, attr->link_create.perf_event.bpf_cookie);
	if (err) {
		bpf_link_cleanup(&link_primer);
		goto out_put_file;
	}
	/* perf_event_set_bpf_prog() does not take a program reference. */
	bpf_prog_inc(prog);
...
static int bpf_prog_attach(const union bpf_attr *attr)
{
	...
	ret = bpf_prog_attach_check_attach_type(prog, attr->attach_type);
	if (ret) {
		bpf_prog_put(prog);
		return ret;
	}
...
static int link_create(const union bpf_attr *attr, bpfptr_t uattr)
{
...
static int link_detach(const union bpf_attr *attr)
{
	struct bpf_link *link;
	int ret;
...
/* Protects shared bpf_stats_enabled_key references. */
...
static int bpf_enable_stats(const union bpf_attr *attr)
{
...
static int bpf_iter_create(const union bpf_attr *attr)
{
	struct bpf_link *link;
	int err;
...
static int bpf_prog_bind_map(const union bpf_attr *attr)
{
	...
	{
		struct fd f = fdget(attr->prog_bind_map.map_fd);
...
BPF_CALL_3(bpf_sys_bpf, int, cmd, void *, attr, u32, attr_size)
{
	...
	case BPF_BTF_LOAD:
		break;
	/* BPF_PROG_TEST_RUN is excluded to prevent recursive test runs. */
	default:
		return -EINVAL;
	}
