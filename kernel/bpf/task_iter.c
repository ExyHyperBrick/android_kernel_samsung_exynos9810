...
task_vma_seq_get_next(struct bpf_iter_seq_task_vma_info *info)
{
	 ...
	 * the task_struct, and holds read lock on vma->mm->mmap_sem.
	...
	if (info->task) {
		curr_task = info->task;
		curr_vma = info->vma;
		/* In case of lock contention, drop mmap_sem to unblock
		 * the writer.
		 *
		...
		if (rwsem_is_contended(&curr_task->mm->mmap_sem)) {
			...
			down_read(&curr_task->mm->mmap_sem);
		} else {
...
again:
		...
		if (curr_tid != info->tid) {
			 ...
			 * We dropped mmap_sem before returning to user
	...
	case task_vma_iter_find_vma:
		/* We dropped mmap_sem so it is necessary to use find_vma
