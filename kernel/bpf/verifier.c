...
static bool arg_type_may_be_null(enum bpf_arg_type type)
{
	return type == ARG_PTR_TO_MAP_VALUE_OR_NULL ||
	       ...
	       type == ARG_PTR_TO_RDONLY_MEM_OR_NULL ||
...
static int release_reference_state(struct bpf_func_state *state, int ptr_id)
{
	...
	for (i = 0; i < state->acquired_refs; i++) {
		if (state->refs[i].id == ptr_id) {
			/* A callback cannot release a caller reference. */
			if (state->in_callback_fn &&
...
static void __reg_combine_64_into_32(struct bpf_reg_state *reg)
{
	...
	if (__reg64_bound_s32(reg->smin_value) &&
	    __reg64_bound_s32(reg->smax_value)) {
...
static int check_subprogs(struct bpf_verifier_env *env)
{
	...
	for (i = 0; i < insn_cnt; i++) {
		if (bpf_pseudo_func(insn + i)) {
			if (!env->bpf_capable) {
				verbose(env,
					"function pointers require CAP_BPF or CAP_SYS_ADMIN\n");
				return -EPERM;
			}
			if (bpf_prog_is_dev_bound(env->prog->aux)) {
				verbose(env,
					"function pointers in offloaded programs are not supported\n");
				return -EINVAL;
			}
		...
		if (!env->bpf_capable) {
			verbose(env,
				"BPF and kernel function calls require CAP_BPF or CAP_SYS_ADMIN\n");
			return -EPERM;
		}
		if (bpf_prog_is_dev_bound(env->prog->aux)) {
	...
	for (;;) {
		...
		for_each_set_bit(i, mask, 64) {
			if (i >= func->allocated_stack / BPF_REG_SIZE) {
				/* A non-canonical stack write can remain in
				 * stack_mask after backtracking a straight-line
				 * block even when the parent never allocated
				 * it.
				 * Fall back to conservative precision tracking.
				 */
				mark_all_scalars_precise(env, st);
				return 0;
			}
				      ...
				      int off, int size, int dst_regno)
{
	...
	if (is_spilled_reg(&reg_state->stack[spi])) {
		...
		for (i = BPF_REG_SIZE - 1;
		     i > 0 && stype[i - 1] == STACK_SPILL; i--)
			spill_size++;
		...
		if (size != BPF_REG_SIZE || spill_size != BPF_REG_SIZE) {
			...
			if (!(off % BPF_REG_SIZE) && size == spill_size) {
				/* check_reg_arg() selected subreg_def already. */
				s32 subreg_def =
					state->regs[dst_regno].subreg_def;
			    ...
			    int ptr_regno, int off, int size, int dst_regno)
{
	...
	/* Variable stack offsets lack Spectre-v1 ALU masking. */
...
{
	return prog->aux->dst_prog ? prog->aux->dst_prog->type :
					     prog->type;
}
...
static int check_ctx_access(struct bpf_verifier_env *env, int insn_idx, int off, int size,
			    ...
			    struct btf **btf, u32 *btf_id, u32 *ctx_ptr_id,
				   ...
				   int value_regno)
{
	...
	if (!btf_vmlinux) {
		verbose(env,
			"map_ptr access requires CONFIG_DEBUG_INFO_BTF\n");
		return -ENOTSUPP;
	}
 ...
 * general reference-tracking logic. The verifier needs to remember only
...
/* Input-only helpers may consume either mutable or immutable buffers. */
static const struct bpf_reg_types readonly_mem_types = {
	.types = {
		PTR_TO_STACK,
		PTR_TO_PACKET,
		PTR_TO_PACKET_META,
...
static const struct bpf_reg_types func_types = {
	.types = { PTR_TO_FUNC },
};
static const struct bpf_reg_types stack_types = {
	.types = { PTR_TO_STACK },
};
...
static const struct bpf_reg_types *compatible_reg_types[__BPF_ARG_TYPE_MAX] = {
	...
	[ARG_PTR_TO_RDONLY_MEM]	= &readonly_mem_types,
	[ARG_PTR_TO_RDONLY_MEM_OR_NULL] = &readonly_mem_types,
			  ...
			  const u32 *arg_btf_id)
{
	...
	if (arg_type >= __BPF_ARG_TYPE_MAX)
		goto unsupported;
...
static int check_func_arg(struct bpf_verifier_env *env, u32 regno,
			  enum bpf_arg_type arg_type, const u32 *btf_id,
			  struct bpf_call_arg_meta *meta)
{
	struct bpf_reg_state *regs = cur_regs(env), *reg = &regs[regno];
	...
	switch (type) {
	case SCALAR_VALUE:
	/* Pointer types where register offsets are explicitly allowed. */
	case PTR_TO_PACKET:
	case PTR_TO_PACKET_META:
	case PTR_TO_MAP_KEY:
	case PTR_TO_MAP_VALUE:
...
force_off_check:
	...
	if (register_is_null(reg) && arg_type_may_be_null(arg_type))
		 ...
		 * the detailed pointer type checks.
		 */
...
skip_type_check:
	...
	if (arg_type == ARG_CONST_MAP_PTR) {
		...
		if (meta->map_ptr) {
			/* Use map_uid (which is the unique id of an inner map) to
			 * reject a timer pointer obtained from one inner map being
			 * initialized with another inner map from the same template.
			 * Comparing map_ptr is enough for normal and outer maps.
			 */
		...
		/* The access to this pointer is checked when the following
		 * memory-size argument is processed.
		 */
...
static bool may_update_sockmap(struct bpf_verifier_env *env, int func_id)
{
	...
	case BPF_PROG_TYPE_SOCK_OPS:
		/* Map update is allowed only through dedicated helpers with
		 * event type checks.
		 */
		return func_id == BPF_FUNC_map_delete_elem;
...
static bool check_refcount_ok(const struct bpf_func_proto *fn, int func_id)
{
	...
	/* A reference acquiring function cannot acquire another
	 * refcounted pointer.
	 */
			 ...
			 bool range_open)
{
	...
	/* The register is pkt > pkt_end or pkt >= pkt_end. How far beyond
	 * pkt_end it goes is unknown. For an open range the pointer is at
	 * least one byte beyond pkt_end; otherwise it can be exactly at it.
	 */
	if (range_open)
			     ...
			     set_callee_state_fn set_callee_state_cb)
{
	...
	if (insn->code == (BPF_JMP | BPF_CALL) &&
		...
		/* Continue with the next instruction after the helper call. */
		return 0;
	}
				       ...
				       int insn_idx)
{
	...
	if (bpf_map_ptr_poisoned(insn_aux)) {
		verbose(env, "map pointer has conflicting values\n");
		return -EINVAL;
	}

	map = BPF_MAP_PTR(insn_aux->map_state);
	if (!map || !map->ops->map_set_for_each_callback_args ||
...
static int prepare_func_exit(struct bpf_verifier_env *env, int *insn_idx)
{
	...
	if (callee->in_callback_fn) {
		...
		/* The callback contract relies on R0's actual value. */
	...
	/* A callback must release its own references before returning, so
	 * its reference state then matches the caller and need not be copied
	 * back. A normal subprogram can transfer its reference state.
	 */
	if (!callee->in_callback_fn) {
...
static int check_get_func_ip(struct bpf_verifier_env *env)
{
	...
	if (env->prog->type == BPF_PROG_TYPE_TRACING) {
		...
		return 0;
	} else if (env->prog->type == BPF_PROG_TYPE_KPROBE) {
		return 0;
	}
			     ...
			     struct bpf_insn *insn, int *insn_idx_p)
{
	...
	if (fn->ret_type == RET_INTEGER) {
		...
		maybe_null = fn->ret_type == RET_PTR_TO_MEM_OR_BTF_ID_OR_NULL;
		mark_reg_known_zero(env, regs, BPF_REG_0);
		t = btf_type_skip_modifiers(meta.ret_btf, meta.ret_btf_id,
					    NULL);
		if (!t) {
		...
		/* Helper prototypes are defined by built-in vmlinux BTF. */
			    ...
			    const bool commit_window)
{
	...
	if (commit_window) {
		...
		/* Explore unknown scalars on every path so a tighter pointer
		 * arithmetic mask cannot be hidden by state pruning.
		 */
		if (!off_is_imm)
...
static void sanitize_mark_insn_seen(struct bpf_verifier_env *env)
{
	...
	/* Speculative paths are verified for safety, but do not make an
	 * otherwise unreachable instruction live. Dead-code sanitization must
	 * still be able to rewrite it from the non-speculative domain.
	 */
	if (!vstate->speculative)
				   ...
				   struct bpf_insn *insn)
{
	...
	if (dst_reg->type != SCALAR_VALUE)
		 ...
		 * incorrectly propagated into other registers by
		 * find_equal_scalars().
		 */
		dst_reg->id = 0;
	if (BPF_SRC(insn->code) == BPF_X) {
		...
		if (BPF_SRC(insn->code) == BPF_X) {
			...
			if (BPF_CLASS(insn->code) == BPF_ALU64) {
				...
				if (src_reg->type == SCALAR_VALUE && !src_reg->id)
					 ...
					 * find_equal_scalars() to propagate the
					 * min/max range.
					 */
				 ...
				 bool is_null)
{
	if (reg_type_may_be_null(reg->type) && reg->id == id &&
		...
		if (is_null) {
			/* NULL no longer carries identity or a reference. */
			reg->id = 0;
			reg->ref_obj_id = 0;
		} else if (!reg_may_point_to_spin_lock(reg)) {
			/* Keep ref_obj_id for release_reference(). */
			reg->id = 0;
		}
	}
}
				  ...
				  bool is_null)
{
	...
	if (ref_obj_id && ref_obj_id == id && is_null)
		/* No one can release the reference before its NULL check. */
			     ...
			     struct bpf_insn *insn, int *insn_idx)
{
	...
	if (pred >= 0) {
		/* A pointer can reach this path only after a known non-NULL
...
static int check_ld_imm(struct bpf_verifier_env *env, struct bpf_insn *insn)
{
	...
	if (insn->src_reg == BPF_PSEUDO_FUNC) {
		...
		if (!prog_aux->func_info ||
			...
			verbose(env, "missing btf func_info for callback\n");
			return -EINVAL;
		}

		type = btf_type_by_id(prog_aux->btf,
				      prog_aux->func_info[subprogno].type_id);
		if (!type || !btf_type_is_func(type) ||
		    BTF_INFO_VLEN(type->info) != 0) {
...
static int check_return_code(struct bpf_verifier_env *env)
{
	...
	if (frame->in_async_callback_fn) {
		struct tnum async_range = tnum_const(0);

		/* Enforce a zero return value from asynchronous callbacks. */
		...
		if (!tnum_in(async_range, reg->var_off)) {
			verbose_invalid_scalar(env, reg, &async_range,
					       "async callback", "R0");
			return -EINVAL;
		}
		return 0;
...
peek_stack:
	...
	if (bpf_pseudo_func(insns + t)) {
		ret = visit_func_call_insn(t, insn_cnt, insns, env, true);
		if (ret == 1)
			goto peek_stack;
		else if (ret < 0)
			goto err_free;
	} else if (BPF_CLASS(insns[t].code) == BPF_JMP ||
		...
		if (opcode == BPF_EXIT) {
			...
			if (insns[t].imm == BPF_FUNC_timer_set_callback)
				/* Mark this call to trigger is_state_visited()
				 * before __check_func_call() pushes another async
				 * callback state for exploration.
				 */
				init_explored_state(env, t);
			ret = visit_func_call_insn(t, insn_cnt, insns, env,
						  insns[t].src_reg ==
						  BPF_PSEUDO_CALL);
			if (ret == 1)
				goto peek_stack;
			  ...
			  bpfptr_t uattr)
{
	...
	for (i = 0; i < nfuncs; i++) {
		...
		if (unlikely(!func_proto ||
			     !btf_type_is_func_proto(func_proto))) {
			/* btf_func_check() already verified the type. */
			ret = -EINVAL;
			goto err_free;
		}
		ret_type = btf_type_skip_modifiers(btf, func_proto->type,
						   NULL);
		scalar_return = btf_type_is_small_int(ret_type) ||
		...
		if (i && !scalar_return &&
		    env->subprog_info[i].has_tail_call) {
			verbose(env,
...
static bool stacksafe(struct bpf_verifier_env *env,
		      struct bpf_func_state *old, struct bpf_func_state *cur,
		      struct bpf_id_pair *idmap)
{
	int i, spi;
...
/* Find precise scalars in the previous equivalent state and propagate them
...
static int is_state_visited(struct bpf_verifier_env *env, int insn_idx)
{
	...
	while (sl) {
		...
		if (sl->state.branches) {
			...
			frame = sl->state.frame[sl->state.curframe];
			if (frame->in_async_callback_fn &&
			    frame->async_entry_cnt !=
			    cur->frame[cur->curframe]->async_entry_cnt) {
				/* A different async_entry_cnt is another entry into
				 * an async callback, not infinite recursion. The older
				 * state is still being explored, so it cannot prune this
				 * one. Loops within one callback retain the same count and
		...
		if (states_equal(env, &sl->state, cur)) {
			...
			/* An equivalent state can differ only in precision
			 * marks. Preserve them before pruning the current path.
			 */
...
next:
	...
	*explored_state(env, insn_idx) = new_sl;
	...
	for (j = 0; j <= cur->curframe; j++) {
		for (i = j < cur->curframe ? BPF_REG_6 : 0;
		     i < BPF_REG_FP; i++)
	...
	for (;;) {
		...
		if (class == BPF_ALU || class == BPF_ALU64) {
			...
			if (opcode == BPF_CALL) {
				...
				/* Callback refs must match the caller before
				 * prepare_func_exit() drops the frame.
				 */
				err = check_reference_leak(env);
				if (err)
					return err;
...
static int find_btf_percpu_datasec(struct btf *btf)
{
	...
	/* Module and vmlinux BTF each have their own percpu DATASEC. */
	n = btf_nr_types(btf);
	i = btf_is_module(btf) ? btf_nr_types(btf_vmlinux) : 1;
...
/* Drop BTF references used by the rejected program. */
					    ...
					    const struct bpf_insn *patch, u32 len)
{
	...
	if (len > 1) {
		u32 new_len = env->prog->len + len - 1;

		new_data = vzalloc(array_size(new_len, sizeof(*new_data)));
		if (!new_data)
			return NULL;
	}
...
static int fixup_bpf_calls(struct bpf_verifier_env *env)
{
	...
	for (i = 0; i < insn_cnt; i++, insn++) {
		...
		if (insn->imm == BPF_FUNC_timer_set_callback) {
			 ...
			 * necessary with different maps and the register states
			 * prepared by set_timer_callback_state will be accurate.
			 *
			 * The following use case is valid:
			 *   map1 is shared by prog1, prog2, prog3.
			 *   prog1 calls bpf_timer_init for some map1 elements
			 *   prog2 calls bpf_timer_set_callback for some map1
			 *     elements. Those that were not bpf_timer_init-ed
			 *     will return -EINVAL.
...
static int do_check_common(struct bpf_verifier_env *env, int subprog)
{
	...
	ret = do_check(env);
out:
	/* do_check() can free cur_state itself under memory pressure. */
	if (env->cur_state) {
		free_verifier_state(env->cur_state, true);
		env->cur_state = NULL;
	}
			    ...
			    struct bpf_attach_target_info *tgt_info)
{
	...
	btf = tgt_prog ? tgt_prog->aux->btf : bpf_prog_get_target_btf(prog);
	if (!btf) {
		bpf_log(log,
...
skip_full_check:
	...
	if (env->used_btf_cnt) {
		/* Preserve the BTF references after verification. */
		env->prog->aux->used_btfs =
			kmalloc_array(env->used_btf_cnt,
				      sizeof(env->used_btfs[0]), GFP_KERNEL);
...
err_release_maps:
	...
	/* Extension programs temporarily inherit the attach type of their
	 * targets for verification, so set it back to zero before returning.
	 */
