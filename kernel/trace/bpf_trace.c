	   ...
	   const void *, data, u32, data_len)
{
	...
	for (i = 0; i < fmt_size; i++) {
		...
		/* fmt[i] != 0 && fmt[last] == 0, so fmt[i + 1] is valid */
		i++;
	...
	/* Pass all possible parameters to seq_printf(). */
...
BPF_CALL_5(bpf_snprintf_btf, char *, str, u32, str_size,
	   struct btf_ptr *, ptr, u32, btf_ptr_size, u64, flags)
{
	const struct btf *btf;
	s32 btf_id;
	int ret;
	...
	return btf_type_snprintf_show(btf, btf_id, ptr->ptr, str,
				      str_size, flags);
}
...
tracing_prog_func_proto(enum bpf_func_id func_id, const struct bpf_prog *prog)
{
	...
	case BPF_FUNC_seq_printf:
		return prog->expected_attach_type == BPF_TRACE_ITER ?
		       &bpf_seq_printf_proto : NULL;
	case BPF_FUNC_seq_write:
		return prog->expected_attach_type == BPF_TRACE_ITER ?
		       &bpf_seq_write_proto : NULL;
	case BPF_FUNC_seq_printf_btf:
		return prog->expected_attach_type == BPF_TRACE_ITER ?
		       &bpf_seq_printf_btf_proto : NULL;
	default:
