		...
		*lru_pages = 0;
		for_each_evictable_lru(lru) {
			...
			if (protection > 0) {
				 ...
				 * proportioning its current usage to
				 * its memory.low or memory.min setting.
				 *
				 * This is important, as otherwise
				 * scanning aggression becomes extremely
				 * binary -- from nothing as we approach
				 * the memory protection threshold, to
				 * totally nominal as we exceed it. This
				 * results in requiring setting
				 * extremely liberal protection
				 * thresholds. It also means we simply
				 * get no protection at all if we set it
				 * too low, which is not ideal.
				 */
