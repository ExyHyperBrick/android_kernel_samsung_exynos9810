...
static struct bpf_local_storage_cache sk_cache = {
	.idx_lock = __SPIN_LOCK_UNLOCKED(sk_cache.idx_lock),
};
...
int bpf_sk_storage_clone(const struct sock *sk, struct sock *newsk)
{
	...
	hlist_for_each_entry_rcu(selem, &sk_storage->list, snode) {
		...
		if (new_sk_storage) {
			...
			bpf_selem_link_storage_nolock(new_sk_storage,
						      copy_selem);
		} else {
...
bpf_sk_storage_diag_alloc(const struct nlattr *nla_stgs)
{
	...
	/* bpf_local_storage_map requires CAP_BPF, as
			    ...
			    unsigned int *res_diag_size)
{
	...
	*res_diag_size = 0;
	...
	saved_len = skb->len;
	for (i = 0; i < diag->nr_maps; i++) {
		sdata = bpf_local_storage_lookup(
			sk_storage,
			(struct bpf_local_storage_map *)diag->maps[i], false);
