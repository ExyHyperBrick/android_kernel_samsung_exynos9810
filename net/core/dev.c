...
static void dev_xdp_set_link(struct net_device *dev,
			     enum bpf_xdp_mode mode,
...
static void dev_xdp_set_prog(struct net_device *dev,
			     enum bpf_xdp_mode mode,
			   ...
			   struct bpf_prog *prog)
{
	...
	/* The driver owns one reference while the core state owns another. */
	if (prog)
		bpf_prog_inc(prog);
	err = bpf_op(dev, &xdp);
	if (err && prog)
			  ...
			  struct bpf_prog *old_prog, u32 flags)
{
	...
	if (new_prog) {
		...
		if (new_prog->expected_attach_type == BPF_XDP_DEVMAP ||
		    new_prog->expected_attach_type == BPF_XDP_CPUMAP)
			return -EINVAL;
	}
...
static int dev_xdp_attach_link(struct net_device *dev,
			       struct bpf_xdp_link *link)
{
	return dev_xdp_attach(dev, link, NULL, NULL, link->flags);
}
...
static void bpf_xdp_link_release(struct bpf_link *link)
{
	...
	xdp_link = container_of(link, struct bpf_xdp_link, link);
...
static void bpf_xdp_link_show_fdinfo(const struct bpf_link *link,
				     struct seq_file *seq)
{
	const struct bpf_xdp_link *xdp_link;
	u32 ifindex = 0;
 ...
 *	@expected_fd: old program fd that userspace expects to replace
		      ...
		      u32 flags)
{
	...
	if (fd >= 0) {
		new_prog = bpf_prog_get_type(fd, BPF_PROG_TYPE_XDP);
		if (IS_ERR(new_prog))
			return PTR_ERR(new_prog);
	}

	if (expected_fd >= 0) {
		old_prog = bpf_prog_get_type(expected_fd, BPF_PROG_TYPE_XDP);
		if (IS_ERR(old_prog)) {
	...
	err = dev_xdp_attach(dev, NULL, new_prog, old_prog, flags);
