...
static bool convert_bpf_ld_abs(struct sock_filter *fp, struct bpf_insn **insnp)
{
	...
	if (!indirect &&
		...
		*insn++ = BPF_JMP_IMM(BPF_JSLT, BPF_REG_TMP, size,
				      2 + endian);
	...
	if (new_prog) {
		...
		*new_insn++ = BPF_MOV64_REG(BPF_REG_CTX, BPF_REG_ARG1);
		if (*seen_ld_abs) {
			 ...
			 */
			*new_insn++ = BPF_LDX_MEM(
				BPF_FIELD_SIZEOF(struct sk_buff, data),
				BPF_REG_D, BPF_REG_CTX,
				offsetof(struct sk_buff, data));
			*new_insn++ = BPF_LDX_MEM(BPF_W, BPF_REG_H,
					      BPF_REG_CTX,
					      offsetof(struct sk_buff, len));
			*new_insn++ = BPF_LDX_MEM(
				BPF_W, BPF_REG_TMP, BPF_REG_CTX,
				offsetof(struct sk_buff, data_len));
			*new_insn++ = BPF_ALU32_REG(BPF_SUB, BPF_REG_H,
					       BPF_REG_TMP);
		}
	} else {
		new_insn += 3;
...
jmp_rest:
		...
		/* ldxb 4 * ([14] & 0xf) is remaped into native insns. */
...
static const struct bpf_func_proto bpf_csum_diff_proto = {
	...
	.pkt_access	= true,
	.ret_type	= RET_INTEGER,
	.arg1_type	= ARG_PTR_TO_RDONLY_MEM_OR_NULL,
	.arg2_type	= ARG_CONST_SIZE_OR_ZERO,
	.arg3_type	= ARG_PTR_TO_RDONLY_MEM_OR_NULL,
			    ...
			    struct net_device *dev, struct bpf_nh_params *nh)
{
	...
	} else if (!ipv6_stub->nd_tbl) {
		rcu_read_unlock_bh();
		goto out_drop;
	} else {
		neigh = __neigh_lookup_errno(ipv6_stub->nd_tbl,
					     &nh->ipv6_nh, dev);
	}
	if (likely(neigh && !IS_ERR(neigh))) {
		__this_cpu_inc(xmit_recursion);
		ret = dst ? dst_neigh_output(dst, neigh, skb) :
...
static int bpf_out_neigh_v4(struct sk_buff *skb, struct net_device *dev,
			    struct bpf_nh_params *nh)
{
	struct dst_entry *dst = NULL;
	u32 hh_len = LL_RESERVED_SPACE(dev);
	...
	if (!nh) {
		...
		neigh = __neigh_lookup_errno(&arp_tbl, &nh->ipv4_nh, dev);
#if IS_ENABLED(CONFIG_IPV6)
	} else if (nh->nh_family == AF_INET6) {
		if (!ipv6_stub->nd_tbl) {
			rcu_read_unlock_bh();
				   ...
				   struct bpf_nh_params *nh)
{
	...
	err = bpf_out_neigh_v4(skb, dev, nh);
	if (unlikely(net_xmit_eval(err)))
		dev->stats.tx_errors++;
	else
		ret = NET_XMIT_SUCCESS;
	   ...
	   int, plen, u64, flags)
{
	...
	BUILD_BUG_ON(sizeof(struct bpf_redir_neigh) !=
...
static const struct bpf_func_proto bpf_redirect_neigh_proto = {
	...
	.ret_type	= RET_INTEGER,
	.arg1_type	= ARG_ANYTHING,
	.arg2_type	= ARG_PTR_TO_RDONLY_MEM_OR_NULL,
	   ...
	   u32, len, u64, flags)
{
	...
	if (!space || (space == 1 && start != offset)) {
		...
		if (append)
			sk_msg_iter_var_prev(copy_i);
		copy = msg->sg.data[copy_i].length;
	}
...
static bool bpf_skb_is_gso_tcp(const struct sk_buff *skb)
{
			    ...
			    u64 flags)
{
	...
	if (skb_is_gso(skb) && !bpf_skb_is_gso_tcp(skb))
		return -ENOTSUPP;
...
static int __xdp_generic_ok_fwd_dev(struct sk_buff *skb,
				    struct net_device *fwd)
{
	unsigned int len;
			    ...
			    struct xdp_buff *xdp, struct bpf_prog *xdp_prog)
{
	...
	err = __xdp_generic_ok_fwd_dev(skb, fwd);
	if (unlikely(err))
		goto err;
...
/* bpf_skc_lookup performs the core lookup for different socket types.
 * It returns request and timewait sockets as well as full sockets.
 */
static struct sock *
		...
		u64 flags, int sdif)
{
	...
	if (sk) {
		...
		/* sk_to_full_sk() may return the request socket's listener.
		 * Drop the original reference before returning that socket.
		 */
		if (!sk_fullsock(sk2))
			sk2 = NULL;
		if (sk2 != sk) {
			sock_gen_put(sk);
			/* The listener must not require a reference bump. */
			if (unlikely(sk2 &&
				     !sock_flag(sk2, SOCK_RCU_FREE))) {
				WARN_ONCE(1,
...
BPF_CALL_5(bpf_tcp_check_syncookie, struct sock *, sk, void *, iph,
	   u32, iph_len, struct tcphdr *, th, u32, th_len)
{
...
BPF_CALL_5(bpf_tcp_gen_syncookie, struct sock *, sk, void *, iph,
	   u32, iph_len, struct tcphdr *, th, u32, th_len)
{
#ifdef CONFIG_SYN_COOKIES
	u32 cookie;
...
BPF_CALL_4(bpf_lwt_in_push_encap, struct sk_buff *, skb, u32, type,
	   void *, hdr, u32, len)
{
	switch (type) {
#if IS_ENABLED(CONFIG_IPV6_SEG6_BPF)
	   ...
	   int, addr_len)
{
	...
	if (addr->sa_family == AF_INET) {
		...
		return __inet_bind(sk, addr, addr_len,
				   BIND_FORCE_ADDRESS_NO_PORT | BIND_FROM_BPF);
#if IS_ENABLED(CONFIG_IPV6)
				    ...
				    u32 *target_size)
{
	...
	case offsetof(struct bpf_xdp_sock, queue_id):
		...
		*insn++ = BPF_LDX_MEM(
			BPF_FIELD_SIZEOF(struct xdp_sock, queue_id),
				      si->dst_reg, si->src_reg,
	...
	*insn++ = BPF_MOV64_REG(BPF_REG_1, BPF_REG_CTX);
	if (!indirect) {
		...
		*insn++ = BPF_MOV64_REG(BPF_REG_2, orig->src_reg);
		if (orig->imm)
			*insn++ = BPF_ALU64_IMM(BPF_ADD, BPF_REG_2,
					       orig->imm);
	}
				      ...
				      struct bpf_insn_access_aux *info)
{
	...
	case bpf_ctx_range(struct bpf_sock_addr, user_port):
		wide_access = bpf_ctx_wide_access_ok(
			off, size,
			offsetof(struct bpf_sock_addr, user_ip6),
			offsetofend(struct bpf_sock_addr, user_ip6)) ||
			bpf_ctx_wide_access_ok(
				off, size,
				offsetof(struct bpf_sock_addr, msg_src_ip6),
				offsetofend(struct bpf_sock_addr, msg_src_ip6));
		if (type == BPF_READ) {
			bpf_ctx_record_field_size(info, size_default);
				  ...
				  struct bpf_insn_access_aux *info)
{
	...
	case bpf_ctx_range_till(struct sk_msg_md,
					     ...
					     u32 *target_size)
{
	...
	case offsetof(struct __sk_buff, data):
		*insn++ = BPF_LDX_MEM(
			BPF_FIELD_SIZEOF(struct bpf_flow_dissector, data),
			si->dst_reg, si->src_reg,
			offsetof(struct bpf_flow_dissector, data));
		break;
	case offsetof(struct __sk_buff, data_end):
		*insn++ = BPF_LDX_MEM(
			BPF_FIELD_SIZEOF(struct bpf_flow_dissector, data_end),
			si->dst_reg, si->src_reg,
			offsetof(struct bpf_flow_dissector, data_end));
		break;
	case offsetof(struct __sk_buff, flow_keys):
		*insn++ = BPF_LDX_MEM(
			BPF_FIELD_SIZEOF(struct bpf_flow_dissector, flow_keys),
			si->dst_reg, si->src_reg,
				...
				struct bpf_prog *prog, u32 *target_size)
{
	...
	case offsetof(struct bpf_sock, rx_queue_mapping):
		/* This older struct sock has no receive-queue cache. */
		*insn++ = BPF_MOV64_IMM(si->dst_reg, -1);
		*target_size = sizeof(__s32);
		break;
	}
					...
					struct bpf_prog *prog, u32 *target_size)
{
	...
	case offsetof(struct bpf_sock_addr, sk):
		*insn++ = BPF_LDX_MEM(
			BPF_FIELD_SIZEOF(struct bpf_sock_addr_kern, sk),
			si->dst_reg, si->src_reg,
						    ...
						    struct bpf_insn *insn)
{
	...
	*insn++ = BPF_ALU64_REG(BPF_SUB, reg, BPF_REG_AX);
	...
	if (si->src_reg == si->dst_reg) {
		...
		*insn++ = BPF_LDX_MEM(BPF_DW, reg, BPF_REG_AX,
				      temp_reg_off);
	}
				    ...
				    struct bpf_prog *prog, u32 *target_size)
{
	...
	case offsetof(struct sk_msg_md, remote_ip6[0]) ...
		...
		off = si->off - offsetof(struct sk_msg_md, remote_ip6[0]);
		*insn++ = BPF_LDX_MEM(BPF_FIELD_SIZEOF(struct sk_msg, sk),
				      si->dst_reg, si->src_reg,
				      offsetof(struct sk_msg, sk));
		*insn++ = BPF_LDX_MEM(
	...
	case offsetof(struct sk_msg_md, local_ip6[0]) ...
		...
		off = si->off - offsetof(struct sk_msg_md, local_ip6[0]);
		*insn++ = BPF_LDX_MEM(BPF_FIELD_SIZEOF(struct sk_msg, sk),
				      si->dst_reg, si->src_reg,
				      offsetof(struct sk_msg, sk));
		*insn++ = BPF_LDX_MEM(
			BPF_W, si->dst_reg, si->dst_reg,
			offsetof(struct sock_common,
				 skc_v6_rcv_saddr.s6_addr32[0]) + off);
#else
		*insn++ = BPF_MOV32_IMM(si->dst_reg, 0);
#endif
		break;
				  ...
				  u32 hash)
{
	...
	bpf_init_reuseport_kern(&reuse_kern, reuse, sk, skb, migrating_sk,
				hash);
	   ...
	   struct sock *, sk, u64, flags)
{
	if (unlikely(flags & ~(BPF_SK_LOOKUP_F_REPLACE |
		...
		return -EINVAL;
	if (unlikely(sk && !sock_flag(sk, SOCK_RCU_FREE)))
		return -ESOCKTNOSUPPORT;
					...
					u32 *target_size)
{
	...
	case offsetof(struct bpf_sk_lookup, sk):
		*insn++ = BPF_LDX_MEM(BPF_SIZEOF(void *), si->dst_reg,
				      si->src_reg,
				      offsetof(struct bpf_sk_lookup_kern,
					       selected_sk));
		break;
