...
static size_t rtnl_xdp_size(void)
{
	size_t xdp_size = nla_total_size(0) +	/* nest IFLA_XDP */
			  ...
			  nla_total_size(4);	/* second per-mode ID */
...
static u32 rtnl_xdp_prog_skb(struct net_device *dev)
{
	...
	return generic_xdp_prog ? generic_xdp_prog->aux->id : 0;
}
...
static int rtnl_xdp_report_one(struct sk_buff *skb, struct net_device *dev,
			       u32 *prog_id, u8 *mode, u8 target_mode,
			       u32 attr,
			       u32 (*get_prog_id)(struct net_device *dev))
{
	...
	current_id = get_prog_id(dev);
	if (!current_id)
		return 0;

	*prog_id = current_id;
	err = nla_put_u32(skb, attr, current_id);
	if (err)
		return err;
...
static int rtnl_xdp_fill(struct sk_buff *skb, struct net_device *dev)
{
	...
	err = rtnl_xdp_report_one(skb, dev, &prog_id, &mode,
				  XDP_ATTACHED_SKB, IFLA_XDP_SKB_PROG_ID,
				  rtnl_xdp_prog_skb);
	if (err)
		goto err_cancel;
	err = rtnl_xdp_report_one(skb, dev, &prog_id, &mode,
	...
	if (err)
		goto err_cancel;
	err = rtnl_xdp_report_one(skb, dev, &prog_id, &mode,
				  XDP_ATTACHED_HW, IFLA_XDP_HW_PROG_ID,
				  rtnl_xdp_prog_hw);
	if (err)
		goto err_cancel;
...
static const struct nla_policy ifla_xdp_policy[IFLA_XDP_MAX + 1] = {
	...
	[IFLA_XDP_DRV_PROG_ID]	= { .type = NLA_U32 },
	[IFLA_XDP_SKB_PROG_ID]	= { .type = NLA_U32 },
	[IFLA_XDP_HW_PROG_ID]	= { .type = NLA_U32 },
};
		      ...
		      struct nlattr **tb, char *ifname, int status)
{
	...
	if (tb[IFLA_XDP]) {
		...
		if (xdp[IFLA_XDP_ATTACHED] || xdp[IFLA_XDP_PROG_ID] ||
		    xdp[IFLA_XDP_DRV_PROG_ID] ||
		    xdp[IFLA_XDP_SKB_PROG_ID] ||
		    xdp[IFLA_XDP_HW_PROG_ID]) {
			err = -EINVAL;
			goto errout;
		}
