			 ...
			 struct sock *sk)
{
	...
	} else if (!stream_verdict && skb_verdict &&
		   !psock->saved_data_ready) {
		sk_psock_start_verdict(sk, psock);
	}
	write_unlock_bh(&sk->sk_callback_lock);
				  ...
				  void *key_next)
{
	...
	elem_next = hlist_entry_safe(
		rcu_dereference(hlist_next_rcu(&elem->node)),
...
static void *sock_hash_seq_find_next(struct sock_hash_seq_info *info,
				     struct bpf_htab_elem *prev_elem)
{
	...
	if (prev_elem) {
		...
		elem = hlist_entry_safe(node, struct bpf_htab_elem, node);
		if (elem)
			return elem;
				      ...
				      struct bpf_iter_aux_info *aux)
{
	...
	bpf_map_inc_with_uref(aux->map);
	info->map = aux->map;
	info->htab = container_of(aux->map, struct bpf_htab, map);
	return 0;
}
			 ...
			 u32 which)
{
	...
	case BPF_SK_SKB_STREAM_PARSER:
		psock_set_prog(&progs->stream_parser, prog);
		break;
	case BPF_SK_SKB_STREAM_VERDICT:
		if (progs->skb_verdict)
			return -EBUSY;
		psock_set_prog(&progs->stream_verdict, prog);
		break;
...
static void sock_map_remove(struct sock *sk, struct sk_psock *psock)
{
	struct sk_psock_link *link;
