...
void reuseport_update_incoming_cpu(struct sock *sk, int val)
{
	...
	if (unlikely(!rcu_access_pointer(sk->sk_reuseport_cb))) {
		/* Paired with READ_ONCE() in sk_incoming_cpu_update()
		 * and compute_score().
		 */
	...
	/* Serialize with reuseport_grow(), which may detach a closed
	 * listener without holding its socket lock.
	 */
						  ...
						  u32 hash, u16 num_socks)
{
	...
	do {
		...
		if (sk->sk_state != TCP_ESTABLISHED) {
			/* Paired with WRITE_ONCE() in the incoming CPU helpers. */
