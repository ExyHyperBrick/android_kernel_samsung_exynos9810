					     ...
					     struct sock *sk)
{
	...
	/* The child socket is locked in inet_csk_listen_stop(). */
...
static void reqsk_timer_handler(unsigned long data)
{
	...
	if (!expire &&
		...
		mod_timer(&req->rsk_timer,
			  jiffies + reqsk_timeout(req, TCP_RTO_MAX));
					 ...
					 struct request_sock *req, bool own_req)
{
	if (own_req) {
		...
		if (sk != req->rsk_listener) {
			...
			/* inet_reqsk_clone() transfers this reference. */
			sock_hold(sk);
	...
	while ((req = reqsk_queue_remove(queue, sk)) != NULL) {
		...
		if (nsk) {
			...
			if (nreq) {
				...
				/* queue_add() already forgot child on failure. */
