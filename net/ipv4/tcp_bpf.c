...
{
	/* Pairs with smp_store_release() after rebuilding the table. */
			   ...
			   sk_read_actor_t recv_actor)
{
	...
	/* Redirected bytes can be acknowledged immediately. Bytes queued
	 * on this socket are acknowledged when recvmsg() consumes them.
	 */
	tp->copied_seq = psock->copied_seq - psock->ingress_bytes;
	tcp_rcv_space_adjust(sk);
	tcp_cleanup_rbuf(sk, copied - psock->ingress_bytes);
out:
	rcu_read_unlock();
	return copied;
}
