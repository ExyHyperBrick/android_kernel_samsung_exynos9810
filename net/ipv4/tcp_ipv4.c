...
process:
	...
	if (sk->sk_state == TCP_NEW_SYN_RECV) {
		...
		if (unlikely(sk->sk_state != TCP_LISTEN)) {
			...
			sk = nsk;
			/* migrate_sock() already retained the listener. */
		} else {
...
/* Find a non-empty bucket starting from st->bucket and return its
 * first matching socket.
 */
...
/* Find the next matching socket in the current listener bucket. If the
 * bucket is exhausted, return the first socket in a later bucket.
 */
...
static int tcp_prog_seq_show(struct bpf_prog *prog,
...
static unsigned int
bpf_iter_tcp_listening_batch(struct seq_file *seq, struct sock *start_sk)
{
...
static unsigned int
bpf_iter_tcp_established_batch(struct seq_file *seq, struct sock *start_sk)
{
	struct bpf_tcp_iter_state *iter = seq->private;
	struct tcp_iter_state *st = &iter->state;
	struct hlist_nulls_node *node;
...
static void *bpf_iter_tcp_seq_next(struct seq_file *seq, void *v,
				   loff_t *pos)
{
	struct bpf_tcp_iter_state *iter = seq->private;
	struct tcp_iter_state *st = &iter->state;
...
static void bpf_iter_tcp_seq_stop(struct seq_file *seq, void *v)
{
	...
	if (!v) {
		...
		if (prog)
			(void)tcp_prog_seq_show(prog, &meta, NULL, 0);
	}
...
{
	tcp_reg_info.ctx_arg_info[0].btf_id =
