			  ...
			  int nonblock, int flags, int *addr_len)
{
	...
	if (sk->sk_family == AF_INET6)
		return udpv6_prot_saved->recvmsg(sk, msg, len, nonblock,
						 flags, addr_len);
#endif
	return udp_prot.recvmsg(sk, msg, len, nonblock, flags, addr_len);
}
...
static void udp_bpf_check_v6_needs_rebuild(struct proto *ops)
{
	if (unlikely(ops != smp_load_acquire(&udpv6_prot_saved))) {
		...
		if (likely(ops != udpv6_prot_saved)) {
			udp_bpf_rebuild_protos(&udp_bpf_prots[UDP_BPF_IPV6],
					       ops);
