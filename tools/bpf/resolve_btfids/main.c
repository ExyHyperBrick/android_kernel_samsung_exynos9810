 ...
 *
 * Keep the ELF patching compatible with this 4.9 tools tree, but use the
 * common libbpf BTF parser imported by the preceding backport series.
 */
#define _GNU_SOURCE
#include <ctype.h>
#include <elf.h>
...
static int btf_find_id(const struct btf *btf, uint32_t want_kind,
                       const char *want_name, uint32_t *result)
{
        uint32_t id, nr_types;

        nr_types = btf__get_nr_types(btf);
        for (id = 1; id <= nr_types; id++) {
...
static struct btf *load_btf(const char *path,
                            const struct elf_view *target)
{
        ...
        if (!path) {
                ...
                if (btf_sec.size > UINT32_MAX)
                        return ERR_PTR(-E2BIG);
                return btf__new(target->file.data + btf_sec.offset,
                                btf_sec.size);
        }
        ...
        if (external.size >= EI_NIDENT &&
            !memcmp(external.data, ELFMAG, SELFMAG)) {
                ...
                err = elf_find_sections(&elf, &btf_sec, &ids, &ids_index,
                                        &symtab);
                if (err || !btf_sec.size || elf.is_be ||
                    btf_sec.size > UINT32_MAX) {
                        elf_close(&elf);
                        return ERR_PTR(err ? err : -EINVAL);
                }
                btf = btf__new(elf.file.data + btf_sec.offset,
                               btf_sec.size);
                elf_close(&elf);
                return btf;
        }

        if (external.size > UINT32_MAX) {
                unmap_file(&external);
                return ERR_PTR(-E2BIG);
        }
        btf = btf__new(external.data, external.size);
        unmap_file(&external);
        return btf;
}
...
int main(int argc, char **argv)
{
	...
	btf = load_btf(btf_path, &elf);
	if (IS_ERR(btf)) {
		fprintf(stderr, "FAILED: cannot load BTF for %s: %s\n",
			btf_path ? btf_path : path,
